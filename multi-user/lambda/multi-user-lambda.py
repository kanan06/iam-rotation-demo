        ...
        for user_name in users_to_rotate:
            try:
                ...
                if len(active_keys) >= 2:
                    print(f"User {user_name} has {len(active_keys)} active keys, deactivating oldest...")
                    # Deactivate oldest key
                    oldest_key = min(active_keys, key=lambda x: x['CreateDate'])
                    iam.update_access_key(
                        UserName=user_name,
                        AccessKeyId=oldest_key['AccessKeyId'],
                        Status='Inactive'
                    )
                    print(f"Deactivated old key: {oldest_key['AccessKeyId']}")
