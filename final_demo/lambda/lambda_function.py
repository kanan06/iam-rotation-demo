...
def get_all_users_from_env_files(env_files):
    ...
    for env_file in env_files:
        try:
            ...
            # The encrypted data is already in binary format, no need to base64 decode
...
def extract_users_from_env_vars(env_vars):
    ...
    # Username mapping from .env format to actual IAM usernames
    username_mapping = {
        'USER1': 'demo-user-1',
        'USER2': 'demo-user-2',
        'USER3': 'demo-user-3',
    ...
    for key, value in env_vars.items():
        if key.startswith('AWS_ACCESS_KEY_ID_'):
            env_username = key.replace('AWS_ACCESS_KEY_ID_', '')
            # Map to actual IAM username
            actual_username = username_mapping.get(env_username, env_username)
            
            if actual_username not in users:
                users[actual_username] = {
                    'access_keys': [],
                    'env_vars': {},
                    'env_username': env_username,  # Keep track of original env username
                    'mapped_from': env_username
                }
            users[actual_username]['access_keys'].append(value)
            users[actual_username]['env_vars'][key] = value
            
            # Look for corresponding secret key
            secret_key = env_vars.get(f'AWS_SECRET_ACCESS_KEY_{env_username}')
            if secret_key:
                users[actual_username]['env_vars'][f'AWS_SECRET_ACCESS_KEY_{env_username}'] = secret_key
